...
def name_to_ticker(user_input):
    user_input = user_input.strip().upper()
    if user_input.endswith('.KS') or user_input.endswith('.KQ'): return user_input
    result = krx_df[(krx_df['Name'] == user_input) | (krx_df['Code'] == user_input)]
    if not result.empty:
    ...
    if len(user_input) == 6 and user_input.isalnum(): return f"{user_input}.KS"
    return None
...
with col1:
    user_input = st.text_input("종목명 또는 티커", value="TIGER 미국우주테크INDXX")
with col2:
...
if st.button("분석 차트 그리기", use_container_width=True):
    ticker = name_to_ticker(user_input)
    if not ticker:
