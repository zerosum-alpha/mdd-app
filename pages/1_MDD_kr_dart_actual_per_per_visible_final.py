...
def build_per_display_chart_df(price_df, per_df):
    """Create a guaranteed PER display frame from price_df + per_df.
    Priority: PER(actual) > PER_PROXY(current EPS proxy).
    This function is intentionally independent from the main dashboard chart,
    so if raw PER_PROXY exists it must appear in this dedicated PER chart.
    """
    if price_df is None or price_df.empty:
    ...
    use_col = None
    label = ""
    if "PER" in p.columns and not p["PER"].dropna().empty:
        use_col = "PER"
        label = "P/E actual"
    elif "PER_PROXY" in p.columns and not p["PER_PROXY"].dropna().empty:
        use_col = "PER_PROXY"
        label = "P/E proxy(current EPS)"

    if use_col is None:
        base["PER_DISPLAY"] = np.nan
        return base, f"PER/PER_PROXY 없음. columns={list(per_df.columns)}"

    src = p[["_Date", use_col]].dropna().sort_values("_Date")
    src = src.drop_duplicates("_Date", keep="last")
    if src.empty:
        base["PER_DISPLAY"] = np.nan
        return base, f"{use_col} 데이터 없음"

    merged = pd.merge_asof(base[["_Date"]].sort_values("_Date"), src, on="_Date", direction="backward")
    base["PER_DISPLAY"] = pd.to_numeric(merged[use_col].to_numpy(), errors="coerce")
    return base, label


def plot_dedicated_per_chart(price_df, per_df, ticker):
    """Dedicated Price + PER chart. It is shown only to verify that PER data is actually drawn."""
    chart, label = build_per_display_chart_df(price_df, per_df)
    if chart.empty or chart["PER_DISPLAY"].dropna().empty:
    ...
    ax2 = ax1.twinx()
    ax2.plot(chart.index, chart["PER_DISPLAY"], color="#D62728", linewidth=2.2, label=label)
    per_vals = chart["PER_DISPLAY"].dropna()
    if not per_vals.empty:
        per_avg = per_vals.mean()
        ax2.axhline(per_avg, color="#D62728", linestyle="--", linewidth=1.0, alpha=0.35, label="P/E avg")
        q05, q95 = per_vals.quantile([0.05, 0.95])
    ...
    fig.tight_layout()
    render_chart(fig)
    st.caption(f"PER 전용 차트 기준: {label}. 원자료에 PER이 없고 PER_PROXY만 있으면 proxy를 표시합니다.")
