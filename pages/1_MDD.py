...
# pykrx는 requirements.txt에 설치되어 있다는 전제로 사용합니다.
# 단, Streamlit Cloud에서 import가 실패하는 경우 원인을 화면에 그대로 표시합니다.
try:
...
if run:
    ...
    if per_available:
        ...
        if market == "KR" and not PYKRX_AVAILABLE:
            st.error("pykrx가 requirements.txt에 있어도 현재 실행 환경에서 import가 실패했습니다. 아래 오류를 확인하세요.")
            st.code(PYKRX_IMPORT_ERROR)
        elif market == "KR":
            st.info("pykrx import는 성공했습니다. 이 경우는 설치 문제가 아니라 KRX PER 데이터 조회 실패/기간 데이터 부족입니다.")
