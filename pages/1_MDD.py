...
# =========================================================
# Practical overlay chart: Price + PER + VIX + signals
# =========================================================
@st.cache_data(ttl=3600)
def load_vix_series(start_date):
    try:
...
def build_overlay_signals(df, profile):
    """
    기존 MDD/RSI 계산값을 이용한 화면 표시용 마커.
    Buy Score 계산에는 영향 없음.
    """
    if df is None or df.empty:
        return pd.DataFrame()
    ...
    required = ["Close", "Current_Drawdown", "RSI"]
    if not all(c in sig.columns for c in required):
        return pd.DataFrame()

    if "Hard_Stop" not in sig.columns:
        sig["Hard_Stop"] = False

    sig["Buy_Signal"] = (
        (sig["Current_Drawdown"] <= profile["buy1"]) &
        (sig["RSI"] <= 38) &
        (~sig["Hard_Stop"].fillna(False))
    )

    sig["Sell_Signal"] = (
        ((sig["Current_Drawdown"] >= -0.03) & (sig["RSI"] >= 68)) |
        ((sig["Current_Drawdown"] >= -0.01) & (sig["RSI"] >= 60))
    )

    # 연속 신호는 첫날만 표시
    prev_buy = sig["Buy_Signal"].shift(1).fillna(False)
    prev_sell = sig["Sell_Signal"].shift(1).fillna(False)

    sig["Buy_Display"] = sig["Close"].where(sig["Buy_Signal"] & (~prev_buy))
    sig["Sell_Display"] = sig["Close"].where(sig["Sell_Signal"] & (~prev_sell))

    out = sig[["Buy_Display", "Sell_Display"]].copy()
...
def make_per_df_from_kr_valuation(kr_val_df):
    if kr_val_df is None or kr_val_df.empty:
        return pd.DataFrame()
    if not {"date", "per"}.issubset(set(kr_val_df.columns)):
        return pd.DataFrame()

    per_df = kr_val_df[["date", "per"]].copy()
    per_df["date"] = pd.to_datetime(per_df["date"], errors="coerce")
    per_df["PER"] = pd.to_numeric(per_df["per"], errors="coerce")
    per_df = per_df.dropna(subset=["date", "PER"])
    per_df = per_df[(per_df["PER"] > 0) & (per_df["PER"] < 300)]
    per_df = per_df.sort_values("date").drop_duplicates(subset=["date"], keep="last")
    if per_df.empty:
        return pd.DataFrame()
    per_df = per_df.set_index("date")[["PER"]]
    return per_df


def make_per_df_from_us_price_pe(price_pe_df):
    if price_pe_df is None or price_pe_df.empty:
        return pd.DataFrame()
    if not {"date", "pe_ttm"}.issubset(set(price_pe_df.columns)):
        return pd.DataFrame()

    per_df = price_pe_df[["date", "pe_ttm"]].copy()
    per_df["date"] = pd.to_datetime(per_df["date"], errors="coerce")
    per_df["PER"] = pd.to_numeric(per_df["pe_ttm"], errors="coerce")
    per_df = per_df.dropna(subset=["date", "PER"])
    per_df = per_df[(per_df["PER"] > 0) & (per_df["PER"] < 300)]
    per_df = per_df.sort_values("date").drop_duplicates(subset=["date"], keep="last")
    if per_df.empty:
...
def make_price_df_for_overlay(price_df):
    if price_df is None or price_df.empty or "Close" not in price_df.columns:
        return pd.DataFrame()
    p = price_df[["Close"]].copy().rename(columns={"Close": "Price"})
...
def plot_price_per_vix_overlay(price_df, per_df, vix_df, signal_df, title):
    """
    사용자가 원한 최종 차트:
    - 왼쪽 축: 주가
    - 오른쪽 축: PER + VIX
    - 주가 위에 BUY / SELL 마커 표시
    """
    price = make_price_df_for_overlay(price_df)
    if price.empty:
        st.warning("주가 데이터가 없어 Price + PER 차트를 표시하지 못했습니다.")
        return pd.DataFrame()
    ...
    if per_df is not None and not per_df.empty:
        ...
        try:
            per.index = per.index.tz_localize(None)
        except Exception:
            try:
                per.index = per.index.tz_convert(None)
            except Exception:
                pass
        if "PER" in per.columns:
            chart_df = chart_df.join(per[["PER"]], how="left")

    if "PER" not in chart_df.columns or chart_df["PER"].dropna().empty:
        st.warning(
            "PER 시계열 데이터가 없어 Price + PER 겹침 차트를 표시하지 못했습니다. "
            "현재 PER 카드만 참고하세요."
        )
        return chart_df

    # PER은 분기/월 단위로 들어올 수 있으므로 최신값 유지
    chart_df["PER"] = chart_df["PER"].ffill()
    chart_df = chart_df.dropna(subset=["Price", "PER"])
    chart_df = chart_df[(chart_df["PER"] > 0) & (chart_df["PER"] < 300)]
    ...
    if vix_df is not None and not vix_df.empty and "VIX" in vix_df.columns:
        ...
        try:
            ...
            try:
                ...
                pass
        chart_df = chart_df.join(v[["VIX"]], how="left")
        chart_df["VIX"] = chart_df["VIX"].ffill()

    if signal_df is not None and not signal_df.empty:
        s = signal_df.copy()
        s.index = pd.to_datetime(s.index)
        chart_df = chart_df.join(s[["Buy_Display", "Sell_Display"]], how="left")

    if len(chart_df) < 5:
        st.warning("Price + PER 차트를 그리기 위한 데이터가 부족합니다.")
        return chart_df

    fig, ax1 = plt.subplots(figsize=(14, 7))

    # Price
    ax1.plot(chart_df.index, chart_df["Price"], linewidth=2.0, label="Price", color="#1F77B4")
    ax1.set_xlabel("Date")
    ax1.set_ylabel("Price", color="#1F77B4")
    ax1.tick_params(axis="y", labelcolor="#1F77B4")
    ax1.grid(True, linestyle=":", alpha=0.55)

    # Signals on price axis
    if "Buy_Display" in chart_df.columns:
        buy = chart_df["Buy_Display"].dropna()
        if not buy.empty:
            ax1.scatter(
                buy.index,
                buy.values * 0.97,
    ...
    if "Sell_Display" in chart_df.columns:
        sell = chart_df["Sell_Display"].dropna()
        if not sell.empty:
            ax1.scatter(
                sell.index,
                sell.values * 1.03,
    ...
    # PER + VIX
    ax2 = ax1.twinx()
    ax2.plot(chart_df.index, chart_df["PER"], linewidth=1.8, linestyle="-", label="P/E", color="crimson")

    per_mean = chart_df["PER"].mean()
    per_std = chart_df["PER"].std()
    if is_valid_number(per_mean):
        ax2.axhline(per_mean, linestyle="-", alpha=0.25, color="crimson", label="P/E avg")
    if is_valid_number(per_std) and per_std > 0:
        ax2.axhline(per_mean + per_std, linestyle=":", alpha=0.25, color="crimson", label="P/E +1SD")
        ax2.axhline(max(per_mean - per_std, 0), linestyle=":", alpha=0.25, color="crimson", label="P/E -1SD")

    if "VIX" in chart_df.columns and chart_df["VIX"].notna().sum() >= 2:
        ax2.plot(chart_df.index, chart_df["VIX"], linewidth=1.1, linestyle="--", alpha=0.6, label="VIX", color="green")

    ax2.set_ylabel("P/E, VIX")
...
def make_price_per_overlay_comment(chart_df, per_label="P/E"):
    if chart_df is None or chart_df.empty or not {"Price", "PER"}.issubset(set(chart_df.columns)):
        return "주가/PER 방향성을 해석할 데이터가 부족합니다."

    df2 = chart_df.dropna(subset=["Price", "PER"]).copy()
    if len(df2) < 5:
        return "주가/PER 방향성을 해석할 데이터가 부족합니다."

    df2 = df2.sort_index()
    cutoff = df2.index.max() - pd.DateOffset(months=6)
    base = df2[df2.index >= cutoff]
    if len(base) < 5:
        base = df2.tail(min(len(df2), 60))
    ...
    if first_price <= 0 or first_per <= 0:
        return "주가/PER 변화율 계산이 불가능합니다."

    price_change = (last_price / first_price - 1) * 100
    per_change = (last_per / first_per - 1) * 100

    if price_change > 0 and per_change < 0:
        msg = "주가는 상승했지만 PER은 하락했습니다. 이익 개선이 주가 상승을 정당화하는 구간일 수 있습니다."
    elif price_change > 0 and per_change > 0:
        msg = "주가와 PER이 함께 상승했습니다. 기대감 선반영 또는 밸류 부담 확대 여부를 확인해야 합니다."
    elif price_change < 0 and per_change < 0:
        msg = "주가와 PER이 함께 하락했습니다. 밸류 부담은 완화됐지만 업황 훼손 여부를 확인해야 합니다."
    elif price_change < 0 and per_change > 0:
        msg = "주가는 하락했지만 PER은 상승했습니다. 이익 하향 또는 실적 둔화 가능성이 있어 저점매수 주의가 필요합니다."
    else:
        msg = "주가/PER 방향성이 뚜렷하지 않습니다."

    current_per = last_per
    per_mean = df2["PER"].mean()
    per_std = df2["PER"].std()
    band_msg = ""
    if is_valid_number(per_mean) and is_valid_number(per_std) and per_std > 0:
        if current_per >= per_mean + 2 * per_std:
            band_msg = "현재 PER은 +2SD 이상으로 과열권입니다."
        elif current_per >= per_mean + per_std:
            band_msg = "현재 PER은 +1SD 이상으로 밸류 부담권입니다."
        elif current_per <= per_mean - per_std:
            band_msg = "현재 PER은 -1SD 이하로 밸류 부담 완화권입니다."
        else:
            band_msg = "현재 PER은 평균권입니다."

    return f"최근 약 6개월 주가 {price_change:+.2f}%, {per_label} {per_change:+.2f}%. {msg} {band_msg}"
...
if run:
    ...
    with st.spinner("데이터 분석 중..."):
        ...
        # =================================================
        # 6. Price + PER + VIX Overlay
        # =================================================
        st.markdown("## 6. Price + PER + VIX Overlay")

        st.info(
            "파란선은 주가, 빨간선은 PER, 초록 점선은 VIX입니다. "
            "주가가 올라도 PER이 내려가면 이익 개선이 주가 상승을 정당화하는 구간일 수 있습니다. "
            "주가가 빠졌는데 PER이 올라가면 이익 악화 가능성을 확인해야 합니다."
        )

        current_vals = get_current_valuation_points(valuation)
        vc1, vc2, vc3, vc4 = st.columns(4)
        vc1.metric("Trailing P/E", format_metric_na(current_vals.get("Trailing P/E")))
        vc2.metric("Forward P/E", format_metric_na(current_vals.get("Forward P/E")))
        vc3.metric("P/S", format_metric_na(current_vals.get("P/S")))
        vc4.metric("PEG", format_metric_na(current_vals.get("PEG")))

        signal_df = build_overlay_signals(df, profile)
        vix_df = load_vix_series(start_date)
        overlay_chart_df = pd.DataFrame()

        if market == "KR":
            st.caption(
                "한국 종목은 pykrx KRX PER 데이터를 사용합니다. Dataguide식 12개월 예상 PER은 아니지만, "
                "일별 주가와 PER 방향성을 함께 보는 용도입니다."
            )

            if krx_stock is None:
                st.error("pykrx가 설치되지 않았습니다. requirements.txt에 pykrx를 추가하세요.")
            else:
                per_df = make_per_df_from_kr_valuation(kr_valuation_trend)
                overlay_chart_df = plot_price_per_vix_overlay(
                    df,
                    per_df,
                    vix_df,
                    signal_df,
                    f"{ticker} Daily Price + PER + VIX"
                )
                if not overlay_chart_df.empty and "PER" in overlay_chart_df.columns and overlay_chart_df["PER"].notna().any():
                    st.write(make_price_per_overlay_comment(overlay_chart_df, "PER"))

                with st.expander("KRX PER/PBR/EPS 원자료 보기"):
                    if kr_valuation_trend is not None and not kr_valuation_trend.empty:
                        ...
                        st.dataframe(show_kr_df[cols].tail(180), use_container_width=True)
                    else:
                        st.info("KRX PER/PBR/EPS 원자료가 없습니다.")

        elif market == "US":
            st.caption(
                "미국 종목은 yfinance 분기 EPS 기반 Estimated TTM P/E를 사용합니다. "
                "FactSet식 과거 12개월 Forward P/E는 아닙니다. 데이터가 부족하면 차트를 억지로 만들지 않습니다."
            )

            per_df = make_per_df_from_us_price_pe(price_pe_df)
            overlay_chart_df = plot_price_per_vix_overlay(
                df,
                per_df,
                vix_df,
                signal_df,
                f"{ticker} Daily Price + Estimated TTM P/E + VIX"
            )
            if not overlay_chart_df.empty and "PER" in overlay_chart_df.columns and overlay_chart_df["PER"].notna().any():
                st.write(make_price_per_overlay_comment(overlay_chart_df, "Estimated TTM P/E"))
            else:
                st.warning(
                    "미국 종목의 PER 변화 차트는 yfinance에서 분기 EPS를 충분히 가져올 때만 표시됩니다. "
                    "현재는 Trailing/Forward P/E 카드만 참고하세요."
                )

            with st.expander("미국 Price + Estimated TTM P/E 원자료 보기"):
                if price_pe_df is not None and not price_pe_df.empty:
                    show_price_pe_df = price_pe_df.copy()
                    show_price_pe_df["date"] = pd.to_datetime(show_price_pe_df["date"]).dt.strftime("%Y-%m-%d")
                    for col in ["price", "eps_ttm", "pe_ttm", "revenue_ttm"]:
                    ...
                    cols = [c for c in ["date", "price", "eps_ttm", "pe_ttm", "revenue_ttm", "source"] if c in show_price_pe_df.columns]
                    st.dataframe(show_price_pe_df[cols].tail(180), use_container_width=True)
                else:
                    st.info("yfinance EPS 기반 PER 원자료가 없습니다.")
        else:
